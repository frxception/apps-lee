      ...
      <v-layout row="row" wrap="wrap">
        <v-flex xs12="xs12" sm10="sm10">
          <v-card flat="flat">
            ...
            <v-card-actions>
              <v-spacer></v-spacer>
              <v-btn flat="flat" @click="release()" :loading="loading" :disabled="!valid">修改文章</v-btn>
            </v-card-actions>
          </v-card>
        </v-flex>
...
import { ALLTAGS, ALLCATEGORY, GETARTICLEBYID } from '@/graphql/query';
import { CREATEARTICLE, UPDATEARTICLE } from '@/graphql/mutation';
  ...
  private async update(): Promise<void> {
    try {
      const result = await this.$apollo.query({
        ...
        variables: {
          id: Number.parseInt(this.$route.params.id, 10),
        },
      });

      this.article.title.value = result.data.getArticleById.title;

      this.article.slug.value = result.data.getArticleById.slug;

      this.article.category.value = result.data.getArticleById.category.id;

      this.article.text.value = result.data.getArticleById.text;

      this.article.html.value = result.data.getArticleById.html;

      this.article.type.value = result.data.getArticleById.type;

      this.article.status.value = Boolean(result.data.getArticleById.status);

      this.article.publish.value = result.data.getArticleById.publish;

      this.article.isTop.value = Boolean(result.data.getArticleById.isTop);

      this.article.allowComment.value = Boolean(
        result.data.getArticleById.allowComment,
      );

      this.article.tags.value = result.data.getArticleById.tags;

      this.article.password.value = result.data.getArticleById.password;
    } catch (error) {
      // console.info(error);
    }
  }
